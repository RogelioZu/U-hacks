...
export default async function PaginaTablero() {
  ...
  if (!profesor) {
    return (
      <div className="space-y-4">
        <h1 className="text-2xl font-bold" style={{ color: "var(--s-navy)" }}>
          Tablero docente
        </h1>
        <div
  ...
  const semanasCerradas =
    todasSemanas?.filter((s) => s.estado === "cerrada") ?? [];
  ...
  const historialSemanas = semanasCerradas.map((semana) => {
    const diags = (todosDiagnosticos ?? []).filter(
      (d) => d.semana_id === semana.id,
    );
    ...
    const pctDom =
      totalAlum > 0
        ? Math.round(
            (diags.filter((d) => d.nivel_dominio >= 2).length / totalAlum) *
              100,
          )
        : null;
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-6">
          ...
          <div>
            ...
            {grupo && (
              <p className="mt-0.5 text-sm text-slate-500">
                {grupo.nombre} ·{" "}
                {semanaActiva
                  ? `Semana ${semanaActiva.numero_semana}`
                  : "Sin semana activa"}
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            {pctDominio !== null && (
              <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <p
                  className={`mt-1 text-3xl font-bold ${pctDominio >= 70 ? "text-green-600" : pctDominio >= 50 ? "text-amber-600" : "text-red-600"}`}
                >
                  {pctDominio}%
                </p>
              </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p
                className={`mt-1 text-3xl font-bold ${alumnosEnRiesgo === 0 ? "text-green-600" : "text-amber-600"}`}
              >
                {alumnosEnRiesgo}
              </p>
            </div>
        ...
        {!semanaActiva && (
          <div className="rounded-xl border border-slate-200 bg-white p-6 text-center">
            ...
            <p className="mt-1 text-sm text-slate-400">
              Cuando el administrador active una semana, aquí verás el
        ...
        {semanaActiva && !aplicacionConfigurada && (
          <div className="rounded-xl border border-slate-200 bg-white p-6 text-center">
            <p className="font-medium text-slate-600">
              Aún no configuras los temas para esta semana.
            </p>
            <p className="mt-1 text-sm text-slate-400">
              El quiz no estará disponible para tus alumnos hasta que lo
              configures.
            </p>
          </div>
        )}
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">
            Semanas anteriores, temas evaluados y evolución del desempeño de tu
            grupo.
          </p>
        </div>
