...
export default async function PaginaDetalleSemana(props: { params: Promise<{ id: string }> }) {
  ...
  const pctDominio = totalAlumnos > 0
    ? Math.round((diagnosticos.filter((d) => d.nivel_dominio >= 2).length / totalAlumnos) * 100)
    : null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">
            {grupo?.nombre} · {new Date(semana.fecha_inicio).toLocaleDateString("es-MX", { month: "long", day: "numeric" })} - {new Date(semana.fecha_fin).toLocaleDateString("es-MX", { month: "long", day: "numeric" })}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {temasVistos.length > 0 ? (
              temasVistos.map((tema, i) => (
                <span key={i} className="inline-flex items-center rounded-md bg-blue-50 px-2.5 py-1 text-sm font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10">
                  {tema}
                </span>
              ))
            ) : (
              <span className="text-sm italic text-slate-400">No se registraron temas para esta semana.</span>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col justify-center items-center text-center">
          ...
          <p className={`mt-2 text-4xl font-bold ${pctDominio && pctDominio >= 70 ? "text-green-600" : pctDominio && pctDominio >= 50 ? "text-amber-600" : "text-red-600"}`}>
      ...
      <div className="space-y-2 mt-8">
        <h2 className="text-base font-semibold text-slate-700">Desempeño por alumno (Archivo Histórico)</h2>
